...
Loop, %runcount%
{
	If firstrun = 0
	{
		...
		; Sends the item action key.
		Send, {2}
		
		Log("ITEM ACTION", "Key 2 sent")
		...
		; Selects a random point inside the configured Item coordinates.
	...
	If firstrun = 0
	{
		...
		; Waits for the configured Blow delay.
		IniRead, sa1, Config.ini, Sleep Blow, min
		IniRead, sa2, Config.ini, Sleep Blow, max
		Random, SleepAmount, %sa1%, %sa2%
		Sleep, %SleepAmount%
		
		Log("SLEEP", "Sleep Blow completed: " SleepAmount " ms")
	}
}
