...
else if (DivinationState = "Find Deposit")
{
	...
	; The deposit rift is the large bright white circular component.
	if LLARS_ColorFindRuneScapeLargeCluster("Divination_IsDepositColor", foundX, foundY, targetScore, targetDensity, 120, 4, 48, 35, 8, 2, 32, 4, "center-biased")
	{
		LLARS_SetStatus("Clicking", "Divination Deposit")
		LLARS_DeveloperAction("Divination || Deposit found || (" . foundX . ", " . foundY . ") || Score=" . targetScore)
		if LLARS_ColorClickDenseTarget(foundX, foundY, "Divination_IsDepositColor", "Divination Deposit", "left", 12, 36, 80, 28, 4)
		{
			DivinationDepositClickTick := A_TickCount
			DivinationDepositCount++
			DivinationDepositSearchLogged := false
			DivinationState := "Deposit Grace"
			LLARS_DeveloperAction("Divination || Deposit clicked || Waiting for action")
		}
	}
}
else if (DivinationState = "Deposit Grace")
{
	...
	; Click deposit, wait three seconds, then use the full-inventory dialog itself as confirmation.
	if (DivinationDepositClickTick && (A_TickCount - DivinationDepositClickTick) >= 3000)
	{
		DivinationDepositClickTick := 0
		DivinationFullDialogScanTick := A_TickCount
		DivinationFullDialogVisible := Divination_FullInventoryDialogVisible()
		...
		if (DivinationFullDialogVisible)
		{
			DivinationDepositSearchLogged := false
			DivinationState := "Find Deposit"
			LLARS_DeveloperAction("Divination || Full inventory dialog still visible after 3 sec || Retry deposit")
		}
		else
		{
			DivinationResourceSearchLogged := false
			DivinationState := "Find Resource"
			LLARS_DeveloperAction("Divination || Deposit cleared full inventory dialog || Find resource")
		}
	}
}
else
