...
Loop, %runcount%
{
	...
	If firstrun = 0
	{
		...
		send {%hkbank%}
		Log("HOTKEY", "Bank Form Preset hotkey sent: " hkbank)
		...
		Random, y, %y1%, %y2%
		NaturalClick(x, y)
		Log("CLICK", "Pottery Menu Coords X=" x " Y=" y)
		...
		Random, y, %y1%, %y2%
		NaturalClick(x, y)
		Log("CLICK", "Pottery Sub-Menu Coords X=" x " Y=" y)
	...
	If firstrun = 0
	{
		...
		Random, SleepAmount, %sa1%, %sa2%
		LLARS_EstimatedSleep(SleepAmount)
		Log("SLEEP", "Sleep Form completed: " SleepAmount " ms")
		...
		if (LLARS_RenewThisLoop)
		{
				...
				IniRead, hk, Config.ini, Renew, bank hotkey
				send {%hk%}
				Log("HOTKEY", "Bank Form Preset hotkey sent: " hk)
