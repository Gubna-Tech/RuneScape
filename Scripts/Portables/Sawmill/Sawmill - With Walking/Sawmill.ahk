; ================================================================
; |     AHK CONFIG     -     AHK CONFIG     -     AHK CONFIG     |
; ================================================================
#Requires AutoHotkey v1.1.37.02
#SingleInstance Force
#Persistent
#InstallKeybdHook
#InstallMouseHook
SetBatchLines, -1

LLARS_SCRIPT_TYPE := "RunCount"

if !LLARS_FrameworkAvailable()
	LLARS_FrameworkError()

LLARS_Initialize()

return

Start:

prime := 0

LLARS_RunCount(Func("Run"))

return

; =========================================================================
; |     >>> BEGIN SCRIPT EDITING <<<     >>> BEGIN SCRIPT EDITING <<<     |
; =========================================================================

; ================================================================
; SCRIPT_EDIT_BEGIN_4C4C415253
; ================================================================

Run(ctx)
{
	global

	if (ctx.IsFirst)
	{
		LLARS_SetStatus("Banking", "Planks")
		LLARS_Click("Bank Prime Coords")

		LLARS_Sleep("Sleep Short")

		LLARS_SetStatus("Loading Preset", "Planks")
		LLARS_PressHotkey("Bank Preset")

		LLARS_Sleep("Sleep Short")

		LLARS_SetStatus("Moving", "Portable Sawmill")
		LLARS_Click("Sawmill Coords")

		LLARS_Sleep("Sleep Walk")

	}
	else
	{
		LLARS_SetStatus("Banking", "Planks")
		LLARS_Click("Bank Main Coords")

		LLARS_Sleep("Sleep Walk")

		LLARS_SetStatus("Loading Preset", "Planks")
		LLARS_PressHotkey("Bank Preset")

		LLARS_RandomSleep()

		LLARS_Sleep("Sleep Short")

		LLARS_SetStatus("Moving", "Portable Sawmill")
		LLARS_Click("Sawmill Coords")

		LLARS_Sleep("Sleep Walk")

	}

	LLARS_SetStatus("Cutting", "Planks")
	type := LLARS_ConfigRead("Plank Type", "Type", "")
	if (type = 1 or type = 2 or type = 3 or type = 4 or type = 5 or type = 6)
	{
		LLARS_CreatorSendInput(type, "Text Input")
	}

	type := LLARS_ConfigRead("Plank Type", "Type", "")
	if (type = 7)
	{
		LLARS_PressKey("8")
		LLARS_Sleep("Sleep Brief")

		LLARS_PressKey("1")
	}

	type := LLARS_ConfigRead("Plank Type", "Type", "")
	if (type = 8)
	{
		LLARS_PressKey("8")
		LLARS_Sleep("Sleep Brief")

		LLARS_PressKey("2")
	}

	type := LLARS_ConfigRead("Plank Type", "Type", "")
	if (type = 9)
	{
		LLARS_PressKey("8")
		LLARS_Sleep("Sleep Brief")

		LLARS_PressKey("3")
	}

	LLARS_Sleep("Sleep Short")

	quantity := LLARS_ConfigReadInteger("Plank Amount", "quantity")
	LLARS_CreatorSendInput(quantity, "Text Input")

	LLARS_Sleep("Sleep Brief")

	LLARS_PressKey("enter")

	LLARS_Sleep("Sleep Short")

	LLARS_PressKey("1")

	LLARS_Sleep("Sleep Normal", true)
}



; ================================================================
; SCRIPT_EDIT_END_4C4C415253
; ================================================================

; ==================================================================
; |     >>> END SCRIPT EDITING <<<     >>> END SCRIPT EDITING <<<  |
; ==================================================================

return
LLARS_FrameworkAvailable()
{
	dir := A_ScriptDir

	Loop, 12
	{
		if (FileExist(dir . "\Core\LLARS.ahk") && FileExist(dir . "\LLARS Config.ini"))
			return true

		SplitPath, dir,, parentDir

		if (parentDir = "" || parentDir = dir)
			break

		dir := parentDir
	}

	return false
}

LLARS_FrameworkError()
{
	Menu, Tray, NoIcon

	MsgBox, 4112, LLARS Error, The complete LLARS project could not be found.`n`nIf you opened this script from a ZIP, RAR, or 7Z archive, extract the entire LLARS folder before running it.

	ExitApp
}

; Automatically searches upward for the LLARS Core bootstrap.
#Include *i %A_ScriptDir%\..\..\Core\LLARS_ScriptBootstrap.ahk
#Include *i %A_ScriptDir%\..\..\..\Core\LLARS_ScriptBootstrap.ahk
#Include *i %A_ScriptDir%\..\..\..\..\Core\LLARS_ScriptBootstrap.ahk
#Include *i %A_ScriptDir%\..\..\..\..\..\Core\LLARS_ScriptBootstrap.ahk
#Include *i %A_ScriptDir%\..\..\..\..\..\..\Core\LLARS_ScriptBootstrap.ahk
#Include *i %A_ScriptDir%\..\..\..\..\..\..\..\Core\LLARS_ScriptBootstrap.ahk
#Include *i %A_ScriptDir%\..\..\..\..\..\..\..\..\Core\LLARS_ScriptBootstrap.ahk
#Include *i %A_ScriptDir%\..\..\..\..\..\..\..\..\..\Core\LLARS_ScriptBootstrap.ahk
#Include *i %A_ScriptDir%\..\..\..\..\..\..\..\..\..\..\Core\LLARS_ScriptBootstrap.ahk
#Include *i %A_ScriptDir%\..\..\..\..\..\..\..\..\..\..\..\Core\LLARS_ScriptBootstrap.ahk
