...
{
	; Keep the low-level keyboard hook installed for LLARS hotkey reliability,
	; but never infer that an injected key was generated by this script.
	; Mouse/keyboard drivers and other programs can also inject keyboard input,
	; so recording those events would mix user/external input into Recent
	; Framework Actions. Script-owned actions are recorded at their source.
