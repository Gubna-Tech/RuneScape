...
{ 	
	If firstrun = 0
	{
		; Makes sure RuneScape is the active window before starting the run.
		...
		; Updates the LLARS GUI with the current run information.
		...
		; Selects a random point inside the configured Smelter coordinates.
		...
		; Sends the configured smelting key.
		send {1}
		
		Log("SMELT KEY", "Key 1 sent")
		
		; Waits for another configured short delay.
		IniRead, sa1, Config.ini, Sleep Short, min
		IniRead, sa2, Config.ini, Sleep Short, max
		Random, SleepAmount, %sa1%, %sa2%
		Sleep, %SleepAmount%		
		...
		; Selects a random point inside the configured bar type coordinates.
	...
	If firstrun = 0
	{
		...
		; Waits for the configured short delay before continuing.
		IniRead, sa1, Config.ini, Sleep Short, min
		IniRead, sa2, Config.ini, Sleep Short, max
		Random, SleepAmount, %sa1%, %sa2%
		Sleep, %SleepAmount%
		...
		if option = true
		{
			...
			; Performs Random Sleep when the random roll is within
			; the configured percentage chance.
			if % RandomNumber <= chance
			{
				...
				; Reads the configured Random Sleep duration.
		...
		; Sends Space to continue the smelting process.
		send {space}
		
		Log("SPACE", "Space key sent")
		
		; Waits for the configured Smelt delay.
		IniRead, sa1, Config.ini, Sleep Smelt, min
		IniRead, sa2, Config.ini, Sleep Smelt, max
		Random, SleepAmount, %sa1%, %sa2%
		Sleep, %SleepAmount%
		
		Log("SMELT WAIT", "Sleep Smelt completed: " SleepAmount " ms")
	}	
}
